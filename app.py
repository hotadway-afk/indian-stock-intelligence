...
try:
    ...
    with tabs[3]:
        ...
        a,b=st.columns(2)
        a.metric("Evidence score", "N/A")
        b.metric("Evidence confidence", "LOW")
        st.markdown("### Verified structured evidence")
        if management.get("evidence"):
            for e in management["evidence"]:
                if isinstance(e, dict):
                    topic = e.get("topic", "Evidence")
                    source = e.get("source", "Source")
                    detail = e.get("detail", "")
                    st.write(f"• **{topic}** — {source}: {detail}")
                else:
                    st.write("• " + str(e))
        else:
            st.info("No structured evidence items were returned.")

        st.markdown("### Extracted document evidence")
        ...
        if findings:
            ...
            st.info("No uploaded company-document evidence was found in this session.")

        st.markdown("### Guidance tracker")
        st.write("FY27 revenue guidance: " + str(management.get("guidance", "Not verified")))
        st.write("FY27 EBITDA / margin guidance: Not verified unless supported by uploaded/company evidence")
        st.write("FY28 revenue guidance: Not verified unless supported by uploaded/company evidence")
        st.write("FY28 EBITDA / margin guidance: Not verified unless supported by uploaded/company evidence")
        st.write("Order book / capex / capacity guidance: Not verified unless supported by uploaded/company evidence")

        st.markdown("### V3.0 evidence discipline")
        st.write(
            "V3.0 does not convert missing management commentary into a neutral management score. "
            "Upload an annual report, investor presentation or earnings-call transcript to improve the evidence score."
        )
