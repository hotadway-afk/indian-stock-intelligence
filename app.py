...
st.set_page_config(
    page_title="Indian Stock Intelligence V2.3",
    page_icon="📊",
    layout="wide",
...
# ============================================================
# V2.3: Unified NSE + BSE + NSE Emerge + BSE SME security resolver
# Coverage targets:
#   - NSE Main Board
#   - NSE Emerge / SME
...
def yahoo_ticker_for(sec):
    exchange=str(sec.get("exchange","NSE")).upper(); symbol=str(sec.get("symbol","")).strip().upper(); code=str(sec.get("bse_code","")).strip()
    if exchange=="BSE": return f"{code}.BO" if code and code.upper() not in {"NAN","NONE"} else f"{symbol}.BO"
    return f"{symbol}.NS"
...
def bse_quote(code):
    try:
        r=requests.get(f"{BSE_API}/getScripHeaderData/w",params={"scripcode":code},headers=REQUEST_HEADERS,timeout=10); r.raise_for_status(); h=r.json().get("Header",{})
...
def load_stock(sec):
    ...
    if sec.get("exchange") == "BSE" and sec.get("bse_code"):
        ...
        if not info.get("currentPrice") and bse_q.get("LTP") is not None: info["currentPrice"]=bse_q.get("LTP")
        if not info.get("regularMarketPrice") and bse_q.get("LTP") is not None: info["regularMarketPrice"]=bse_q.get("LTP")
        if not info.get("longName"): info["longName"]=bse_q.get("Scrip_Name") or bse_q.get("ScripName") or bse_q.get("CompanyName")
        if not sec.get("isin") and bse_q.get("ISIN"): sec["isin"]=str(bse_q["ISIN"]).strip().upper()
...
def management_engine(sec, info):
    ...
    return {
        ...
        "note": "V2.3 does not manufacture FY27/FY28 guidance. Add NSE/BSE filings, investor presentations and earnings-call transcripts for a company-specific management score."
    }
...
# UI
# -----------------------------
st.title("📊 Indian Stock Intelligence — V2.3")
st.caption("Unified NSE + BSE + NSE Emerge + BSE SME Fundamental + Evidence + Valuation + Technical + Risk engine")
...
with st.sidebar:
    ...
    run = st.button("🚀 RUN V2.3 ANALYSIS", type="primary", use_container_width=True)

if not run:
    st.info("Enter a symbol, BSE code, ISIN or company name and click RUN V2.3 ANALYSIS.")
...
try:
    ...
    if hist.empty:
        st.error("The security was resolved from the exchange universe, but no usable historical market-data series was returned by the available providers.")
        st.warning("This is a market-data coverage limitation — NOT a statement that the company is unlisted.")
        st.dataframe(pd.DataFrame([{"Exchange":sec.get("exchange"),"Segment":sec.get("segment"),"Symbol":sec.get("symbol"),"BSE code":sec.get("bse_code"),"ISIN":sec.get("isin"),"Yahoo ticker attempted":info.get("providerTicker"),"Universe source":sec.get("source")}]),use_container_width=True,hide_index=True)
        st.stop()
    ...
    with st.expander("🔎 Security identity / exchange verification", expanded=True):
        ...
        st.caption("Exchange identity is resolved separately from third-party market-data coverage.")

    cols=st.columns(6)
    cols[0].metric("V2.3 Score", f"{overall:.0f}/100")
    ...
    st.divider()
    st.caption("V2.3 is an analytical prototype, not investment advice. Exchange universe discovery uses official NSE/BSE sources; third-party financial data can be delayed, incomplete or unavailable for some SME securities. Verify material decisions against exchange/company filings.")
